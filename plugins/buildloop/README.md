...
- **Enforcement** — twelve skills that drive the four phases.
- **Auditing** — four review agents.
...
agents/
  think-gate    — read-only ① → ② gate; validates the fc (§6.2)
  plan-gate     — read-only ② → ③ gate; validates the bp (§6.3), folds in a simplify pass
...
4. **Add your project-specific gate agents** (optional). The plugin ships the generic `code-checker`, `think-gate`, `plan-gate`, and `ux-checker`. If your stack needs more (e.g. a `security-reviewer`), add it under `.claude/agents/` and name it in `AGENTS.md §6.1`.
