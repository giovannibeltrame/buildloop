...
Filter the candidate before it costs anything to plan. Apply the writing principles ([AGENTS.md §4.7](AGENTS.md)); never guess (§4.6).

## Steps

1. Read the fc with its prototypes ([§3.5](AGENTS.md)). If prototypes are missing, refuse and name what's needed.
2. **Questions loop:** Is it worth building (per prototype)? Which prototype best conveys the narrative? Resolve open questions; if any remain, capture them and stop — a verdict on a guess is forbidden (§4.6).
