...
The **single writer** of the `## Log` table. That table is the only source of truth for "where are we"; there is no `Status:` field. Apply the log-table state model (`${CLAUDE_PLUGIN_ROOT}/AGENTS.md` §2.2); do not restate it.
