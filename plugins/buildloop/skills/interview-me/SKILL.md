...
Open phase ① by turning an idea into a feature candidate. Guard the WHAT with the anti-assumption discipline the `ddd` skill owns — never guess, always ask. Apply the writing principles (AGENTS.md §3.4); do not restate any rule.
...
1. **New or re-sharpen?** If re-sharpening an existing fc (e.g. `does-it-worth` returned "not yet"), open it. Otherwise create one:
   ...
   ```
   Copy the plugin's `templates/fc-xxxx.md` to `docs/buildloop/working/fc-NNNN.md` (per the doc map, §2.5).
2. **Run the questions loop**, one question per ambiguity — never guess. Read the product docs the idea touches (§1) first. Fill the fc sections:
   ...
   - **Metrics** — for each hypothesis, the metric, its success threshold, and the app **event/counter** that emits it (the telemetry seam, §3.5 — naming it here is what lets `measure` judge it later).
