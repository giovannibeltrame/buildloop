...
Judge the bet. Compare the launched cohort's live metrics to the thresholds set in `interview-me`, and decide rollout. This closes the hypothesis loop ([AGENTS.md §4.10](AGENTS.md)). Apply the writing principles ([§4.7](AGENTS.md)).
...
2. Read the cohort's **live prod data** from the telemetry sink and compare each metric to its threshold. [PROJECT: name the dashboard / query / metrics store, per §4.10.]
3. Record the **verdict** in the feature-document's metrics table, one of:
   ...
   - **invalidated** — threshold clearly missed → the premise was wrong. Re-enter the loop: `buildloop log <feature-doc> "measure · re-think" "invalidated" --to "① (Re)Think it"`, then hand off to `/buildloop:interview-me` to re-sharpen (or drop).
4. Update the verdict column and log the outcome.

## Notes

- The chain that feeds this: metric + threshold defined in ① (fc), instrumented in ③ (tdd), released to a cohort in ④ (ship-in-prd), judged here in ④ (§4.10).
- Rollout and rollback are flag flips, not redeploys (§4.9).
