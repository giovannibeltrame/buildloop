...
4. **Pin telemetry.** Where a hypothesis from the fc maps to a metric, add an acceptance row asserting the app **emits that metric's event/counter** (the telemetry seam, §1.8), so instrumentation ships with the feature rather than bolted on later.
