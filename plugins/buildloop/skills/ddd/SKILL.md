...
Guard the WHAT. Define the domain in the bp's `## WHAT — domain (DDD)` section. This skill owns the **DDD anti-assumption discipline**. Apply the writing principles (AGENTS.md §2).
...
3. Define **models, rules, services**: the entities, invariants, services, and boundaries the feature needs. Name what gets built, not just the behavior.
4. For every ambiguity, write **one question** into an `## Open Questions` section. Resolve or surface them; do not paper over them.
