...
The coverage rule is the single 100%-line rule in [AGENTS.md §4.4](AGENTS.md) — no tiers. When the diff adds a **new source file** under a monitored root, your call is binary:
...
- **Non-product** — process entries / thin CLI shims, framework config, generated files, and whatever the project's non-product list (the §4.4 single source of truth the gate reads) names. Decide by the definition, then add the path to that list with a one-line justification.
...
Per [AGENTS.md §4.5](AGENTS.md) and the commit format in [§5.1](AGENTS.md): grep the change's commits for `RED —` / `GREEN —` tags and flag unpaired RED entries. For a log-declared characterization fix (§4.5 exemption), look instead for the coverage-gate red→green plus a recorded mutation spot-check. Confirm every phase transition has a matching `## Log` row ([§3.2](AGENTS.md)).
...
- New file: path → product (gate to 100%) | non-product — one-line justification per §4.4.
