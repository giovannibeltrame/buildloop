...
Before the rubric, derive the fc's phase:
...
- `none`: no transition logged yet, so the fc is malformed. Example: `think-gate: fc-0030 has no logged phase; interview-me must write the creation row first.`
...
Read the first non-metadata line after the H1. Confirm exactly one sentence with no `and`, no `&`, no comma-joined deliverables, not running across sentences. Reject otherwise (§1).
...
### Criterion 5: metrics
Confirm a `## Metrics` table where **every hypothesis** has a row giving a metric, a success threshold, and the **event/counter** that emits it (the telemetry seam, §2.8). Reject a hypothesis with no measurable metric, a metric with no threshold, or a metric with no named emitter.
...
Return a single structured report to the caller, nothing else (no doc write):
...
**FLAG example.** An fc whose `## Metrics` lists "improve engagement" with no threshold and no emitting event. Verdict: `fail`. Gaps: `[{ "criterion": 5, "location": "fc-0030.md:§Metrics", "concrete_fix": "Give the hypothesis a threshold and name the event/counter that emits it, per AGENTS.md §2.8." }]`.
